    ...
    <view class="video-grid" v-if="items.length > 0">
      <view v-for="v in items" :key="v.id" class="video-card" @click="onCardClick(v)">
        <view class="cover-wrap">
          ...
          <view class="badge-tag" v-if="v.progress && v.progress < 1">进行中</view>
        ...
        <view class="info">
          ...
          <view class="meta">
            <text class="meta-text">{{ formatProgress(v.progress) }}</text>
          </view>
        </view>
      </view>
    </view>
...
const handleBulkDelete = () => {
  ...
  uni.showModal({
    title: '批量移除历史',
    content: `确定要移除这 ${selectedIds.value.length} 条记录吗？`,
    confirmColor: '#ee0a24',
    success: async (res) => {
      if (res.confirm) {
        try {
          await request({
            url: '/api/interactions/history/bulk-remove/',
            method: 'POST',
...
const fetchList = async (refresh = false) => {
  ...
  try {
    const res: any = await request({
      url: `/api/interactions/history/?page=${page.value}&page_size=12`,
    })
...
const formatProgress = (p: any) => {
  const n = Number(p)
  if (!Number.isFinite(n)) return '观看记录'
