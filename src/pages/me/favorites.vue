    ...
    <view class="video-grid" v-if="items.length > 0">
      <view v-for="v in items" :key="v.id" class="video-card" @click="onCardClick(v)">
        ...
        <view class="info">
          ...
          <view class="meta">
            <van-icon name="star-o" size="12px" color="#9499a0" />
            <text class="meta-text ml-4">已收藏</text>
          </view>
        </view>
      </view>
    </view>
...
const handleBulkDelete = () => {
  ...
  uni.showModal({
    title: '批量取消收藏',
    content: `确定要取消收藏这 ${selectedIds.value.length} 个内容吗？`,
    confirmColor: '#ee0a24',
    success: async (res) => {
      if (res.confirm) {
        try {
          await request({
            url: '/api/interactions/favorites/bulk-remove/',
            method: 'POST',
...
const fetchList = async (refresh = false) => {
  ...
  try {
    const res: any = await request({
      url: `/api/interactions/favorites/?page=${page.value}&page_size=12`,
    })
