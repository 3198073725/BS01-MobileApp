<template>
  <view :class="['page', theme]">
    <view class="nav-bar">
      ...
      <text class="nav-title">稍后再看</text>
    ...
    <view class="video-grid" v-if="items.length > 0">
      <view v-for="v in items" :key="v.id" class="video-card" @click="onCardClick(v)">
        ...
        <view class="info">
          ...
          <view class="meta">
            <van-icon name="clock-o" size="12px" color="#9499a0" />
            <text class="meta-text ml-4">稍后看</text>
          </view>
        </view>
      </view>
    </view>
...
const handleBulkDelete = () => {
  ...
  uni.showModal({
    title: '批量移除',
    content: `确定要从稍后再看中移除这 ${selectedIds.value.length} 个内容吗？`,
    confirmColor: '#ee0a24',
    success: async (res) => {
      if (res.confirm) {
        try {
          await request({
            url: '/api/interactions/watch-later/bulk-remove/',
            method: 'POST',
...
const fetchList = async (refresh = false) => {
  ...
  try {
    const res: any = await request({
      url: `/api/interactions/watch-later/?page=${page.value}&page_size=12`,
    })
    ...
    if (refresh || !items.value.length) {
      uni.showToast({ title: '稍后再看加载失败', icon: 'none' })
    }
  } finally {
    loading.value = false
